...
import { isValidBitcoinPrivateKey, isValidBitsharesPrivateKey, isValidBitcoinPublicKey } from "../web/src/pkg/address/validator"
...
export async function getUserInput(): Promise<ACCSFields> {
    ...
    fields.mode = (await askUser("Are you proposer or accepter? (proposer/accepter): ")).toLowerCase()
    if (fields.mode !== "proposer" && fields.mode !== "accepter") {
        throw new Error("Invalid mode. Choose proposer or accepter")
    }

    fields.networkToTrade = (await askUser("On what network do you want to trade? (mainnet/testnet): ")).toLowerCase()
    if (fields.networkToTrade !== "mainnet" && fields.networkToTrade !== "testnet") {
        throw new Error("Invalid network. Choose mainnet or testnet.")
    }

    fields.currencyToGive = await askUser("Would you like to give BTC or BTS? (BTC/BTS): ")
    ...
    const priority = await askUser("Please enter the priority of the Bitcoin transactions (0 = high, 1 = medium, 2 = low): ")
    ...
    fields.bitcoinPrivateKey = await askUser("Please enter your Bitcoin private key (WIF format): ")
    if (!isValidBitcoinPrivateKey(fields.bitcoinPrivateKey, fields.networkToTrade)) {
        throw new Error("Invalid Bitcoin private key.")
    }

    fields.bitsharesPrivateKey = await askUser("Please enter your Bitshares private key (WIF format): ")
    if (!isValidBitsharesPrivateKey(fields.bitsharesPrivateKey)) {
        throw new Error("Invalid Bitshares private key.")
    }

    fields.counterpartyBitcoinPublicKey = await askUser("Please enter the Bitcoin public key of the counterparty: ")
    if (!isValidBitcoinPublicKey(fields.counterpartyBitcoinPublicKey)) {
        throw new Error("Invalid Bitcoin public key.")
    }

    fields.counterpartyBitsharesAccountName = await askUser("Please enter the Bitshares account name of the counterparty: ")

    const amount = await askUser(
        `Please enter the amount of ${txTypeName.slice(-3)} you want to exchange: `,
    )

    const exchangeRate = await askUser("Please enter the exchange rate in BTS/BTC: ")
    ...
    console.log(
        `You will get ${fields.amountToReceive} ${txTypeName.substring(
            0,
            3,
        )} for giving ${fields.amountToSend} ${txTypeName.slice(-3)}.`,
    )

    let timelock: string

    if (fields.mode === "proposer") {
        timelock = await askUser(
        ...
        timelock = await askUser("Please enter the duration of the timelock you received from the proposer: ")
        ...
        // Get secret hash
        const hashString = await askUser("Please enter the secret hash you received from the proposer: ")
