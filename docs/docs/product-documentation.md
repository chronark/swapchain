...
As a user of Swapchain, I want to exchange BTC for BTS or vice versa without trusting the other party. Also I do not want my private keys to be stored by the system and I do not want to rely on a third party (Zipkin, 2020 [2]).

#### 2.1.6. User interaction and design

The system communicates with the users via a web app.
Furthermore, the users can utilise Swapchain's services via a command-line tool.
Both are intuitive to use and compatible with each other.
...
Flexibility:
The users can choose the cryptocurrencies they wish to swap, as well as their desired timelock length and also the fees (resulting in shorter or longer transaction durations). Thus, they are granted a high degree of flexibility while trading.
...
A typical user persona for the Swapchain would be Bob/Alice. Bob/Alice, 27 years old, just recently completed their educational course. Two months ago, he/she entered professional life in a corporation. As he/she just recently began to earn a regular wage, he/she is looking for profitable yet serious investment opportunities such as stocks, funds or comparable. Moreover, being well-educated with cryptocurrencies and their underlying technologies and having traded them for quite a while, he/she is also planning to invest into digital assets given they are safe and reliable. Therefore, a sufficient level of security is inevitable to persuade Bob/Alice to invest in these assets and the underlying ACCS technology. Moreover, Bob/Alice is concerned with data protection and thus convinced that even when it comes to web-based transactions, as much information as possible should remain in his/her area of influence without being forwarded and stored by third parties.
...
Unit tests are implemented in jest and a 65% code line coverage is enforced as pre-commit hook. To ensure code quality and standards we use eslint with prettier. We test against the node LTS version (12) as well as the current 14.x version.
...
2. Enter the hash that was previously passed to you by your exchange partner that set up the ACCS.
...
In any case of system errors or malfunctions, please do not hesitate to contact us or state an issue.
Further information: https://github.com/chronark/swapchain/issues.
