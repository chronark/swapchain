...
export const Table = (props: Props) => {
  ...
  const toggleSortByAmountYouPay = () => {
    switch (sortedByAmountYouPay) {
      ...
      case 0:
        orders.sort((a: Order, b: Order) => {
          return b.give.value - a.give.value
        })
        setSortedByAmountYouPay(1)
        break
      case 1:
  ...
  return (
    <div className="flex-grow min-w-full bg-white ">
      <div className="flex items-center justify-between">
        ...
        <div
          className={` flex items-center px-4 py-2 mr-8 bg-yellow-100 rounded transition duration-300 transform ease-in-out ${
            orders.length > 0 ? "opacity-0 translate-x-20" : "opacity-100"
          }`}
        >
          ...
          <table className="min-w-full table-auto">
                  ...
                  <div className="flex">
                    ...
                    {sortedByDate === 1 ? (
                      <ChevronUp
                        ...
                        className="w-4 h-4 ml-1 text-gray-600 hover:text-gray-900"
                      ></ChevronUp>
                    ) : (
                      <ChevronDown
                        onClick={toggleSortByDate}
                ...
                <th className="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-700 uppercase bg-white-100">
                  <div className="flex">
                    ...
                    {sortedByAmountYouPay === 1 ? (
                      <ChevronUp
                        onClick={toggleSortByAmountYouPay}
                        className="w-4 h-4 ml-1 text-gray-600 hover:text-gray-900"
                      ></ChevronUp>
                    ) : (
                      <ChevronDown
            ...
            <tbody className="bg-white">
              {orders.map((order, key) => {
                return <Row key={key} order={order} onClick={onRowClick}></Row>
              })}
            </tbody>
          </table>
        </div>
